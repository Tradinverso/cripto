        ...
        {activeView === "panel" && <>
          <section className="finance-grid" aria-label="Resumen financiero mensual">
            <article className="panel finance-panel euro-panel">
              <div className="panel-heading finance-heading"><div><p className="finance-kicker">EUROS · BIZUM / STRIPE</p><h2>Entradas y previsión</h2><p>Histórico reciente y cobros programados</p></div><div className="month-summary"><span><small>Este mes</small><strong>{(monthlyFinance.current?.euroCollected || 0).toLocaleString("es-ES")} €</strong></span><span><small>Próximo mes</small><strong>{(monthlyFinance.next?.euroExpected || 0).toLocaleString("es-ES")} €</strong></span></div></div>
          ...
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="new-student-title">
            ...
            <form onSubmit={createStudent}>
              ...
              <div className="form-section"><h3>Plan y forma de pago</h3><div className="form-grid">
                <label>Plan<select value={form.plan} onChange={(e) => { const plan = Number(e.target.value); setForm({ ...form, plan, installmentAmount: plan === 2 ? 750 : plan === 5 ? 320 : 550 }); }}><option value={2}>2 pagos</option><option value={3}>3 pagos</option>{form.currency === "EUR" && form.network === "Stripe" ? <option value={5}>5 pagos · Stripe</option> : null}</select></label>
                <label>Importe de cada pago<input readOnly={form.plan === 5} required type="number" min="1" value={form.installmentAmount} onChange={(e) => setForm({ ...form, installmentAmount: Number(e.target.value) || 0 })} /></label>
                ...
                {form.currency === "EUR" ? <label>Método de pago<select value={form.network} onChange={(e) => { const network = e.target.value; const plan = network === "Stripe" ? form.plan : form.plan === 2 ? 2 : 3; setForm({ ...form, network, plan, installmentAmount: plan === 2 ? 750 : plan === 5 ? 320 : 550 }); }}><option>Bizum</option><option>Stripe</option></select></label> : <label>Red configurada<input readOnly value={form.network} /></label>}
                ...
                <div className={`plan-price-summary wide ${form.currency === "EUR" ? "euro" : "crypto"}`}><span>{form.currency === "EUR" ? "€" : "$"}</span><div><strong>{`${form.plan} cuotas de ${form.installmentAmount || 0} ${form.currency === "EUR" ? "€" : "USD"} · Total ${(form.installmentAmount || 0) * form.plan} ${form.currency === "EUR" ? "€" : "USD"}`}</strong><small>{form.plan === 5 ? "Modalidad especial mediante Stripe." : form.plan === 2 ? "Modalidad en dos cuotas con acceso completo tras el segundo pago." : form.currency === "EUR" ? "Desglose interno: precio base 1.497 € + 153 € de financiación (51 € por cuota)." : "El importe en criptomonedas puede adaptarse a cada alumno."}</small></div></div>
              </div></div>
