          ...
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="new-student-title">
            ...
            <form onSubmit={createStudent}>
              ...
              <div className="form-section"><h3>Plan y forma de pago</h3><div className="form-grid">
                <label>Plan<select value={form.plan} onChange={(e) => { const plan = Number(e.target.value); setForm({ ...form, plan, installmentAmount: plan === 5 ? 320 : 550 }); }}><option value={3}>3 pagos</option>{form.currency === "EUR" && form.network === "Stripe" ? <option value={5}>5 pagos · Stripe</option> : null}</select></label>
                ...
                {form.currency === "EUR" ? <label>Método de pago<select value={form.network} onChange={(e) => { const network = e.target.value; setForm({ ...form, network, plan: network === "Stripe" ? form.plan : 3, installmentAmount: network === "Stripe" && form.plan === 5 ? 320 : 550 }); }}><option>Bizum</option><option>Stripe</option></select></label> : <label>Red configurada<input readOnly value={form.network} /></label>}
                ...
                <div className={`plan-price-summary wide ${form.currency === "EUR" ? "euro" : "crypto"}`}><span>{form.currency === "EUR" ? "€" : "$"}</span><div><strong>{form.plan === 5 ? "5 cuotas de 320 € · Total 1.600 €" : `3 cuotas de ${form.installmentAmount || 0} ${form.currency === "EUR" ? "€" : "USD"} · Total ${(form.installmentAmount || 0) * 3} ${form.currency === "EUR" ? "€" : "USD"}`}</strong><small>{form.plan === 5 ? "Modalidad especial mediante Stripe." : form.currency === "EUR" ? "Desglose interno: precio base 1.497 € + 153 € de financiación (51 € por cuota)." : "El importe en criptomonedas puede adaptarse a cada alumno."}</small></div></div>
              </div></div>
