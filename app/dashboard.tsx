          ...
          <section className="modal" role="dialog" aria-modal="true" aria-labelledby="new-student-title">
            ...
            <form onSubmit={createStudent}>
              ...
              <div className="form-section"><h3>Plan y forma de pago</h3><div className="form-grid">
                ...
                {form.currency === "EUR" ? <label>Método de pago<select value={form.network} onChange={(e) => { const network = e.target.value; setForm({ ...form, network, plan: network === "Stripe" && form.plan === 5 ? 5 : 3, installmentAmount: network === "Stripe" && form.plan === 5 ? 320 : 550 }); }}><option>Bizum</option><option>Stripe</option></select></label> : <label>Red configurada<input readOnly value={form.network} /></label>}
                ...
                <div className={`plan-price-summary wide ${form.currency === "EUR" ? "euro" : "crypto"}`}><span>{form.currency === "EUR" ? "€" : "$"}</span><div><strong>{form.plan === 5 ? "5 cuotas de 320 € · Total 1.600 €" : `3 cuotas de 550 ${form.currency === "EUR" ? "€" : "USD"} · Total 1.650 ${form.currency === "EUR" ? "€" : "USD"}`}</strong><small>{form.plan === 5 ? "Modalidad especial mediante Stripe." : form.currency === "EUR" ? "Desglose interno: precio base 1.497 € + 153 € de financiación (51 € por cuota)." : "Plan estándar para pagos mediante USDT o USDC."}</small></div></div>
              </div></div>
