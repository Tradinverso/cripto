...
export async function createPandaDocContract(student:ContractStudent,payments:ContractPayment[],provider:Provider) {
  ...
  const doc=new Document({creator:"TRADINVERSO",title:`Acuerdo privado - ${student.fullName}`,description:"Acuerdo privado de acceso y pago del programa TRADINVERSO",styles:{default:{document:{run:{font:"Arial",size:19,color:NAVY},paragraph:{spacing:{after:100,line:278}}},heading1:{run:{font:"Arial",size:26,bold:true,color:BLUE},paragraph:{spacing:{before:260,after:120}}}}},sections:[{
    ...
    children:[
      ...
      new Table({width:{size:9440,type:WidthType.DXA},columnWidths:[4720,4720],borders,rows:[new TableRow({children:[
        cell([new Paragraph({children:[run("TRADINVERSO / PRESTADOR",{bold:true,color:BLUE,size:16})],spacing:{after:80}}),new Paragraph({children:[run(`${provider.name} · DNI ${provider.documentId}`,{size:17})],spacing:{after:80}}),new Paragraph({children:[run("[signature:provider:p_sig____________]",{color:"FFFFFF",size:16})],spacing:{after:90}}),new Paragraph({children:[run("[date:provider:p_date________]",{color:"FFFFFF",size:16})]})],4720),
        cell([new Paragraph({children:[run("EL ALUMNO",{bold:true,color:BLUE,size:16})],spacing:{after:80}}),new Paragraph({children:[run(`${student.fullName} · DNI / Pasaporte ${student.documentId}`,{size:17})],spacing:{after:80}}),new Paragraph({children:[run("[signature:student:s_sig________________]",{color:"FFFFFF",size:16})],spacing:{after:90}}),new Paragraph({children:[run("[date:student:s_date________]",{color:"FFFFFF",size:16})]})],4720),
      ]})]}),
    ],
  }]});
